...
async function runMigrations(connection) {
  ...
  await ensureColumn(connection, 'restaurants', 'is_open', 'is_open TINYINT(1) DEFAULT 1');
  ...
  try {
    await connection.query(`CREATE INDEX idx_password_reset_user_created ON password_reset_codes (user_id, created_at)`);
  } catch {}
  ...
  try {
    await connection.query(`ALTER TABLE restaurants MODIFY price_level VARCHAR(20) DEFAULT 'Medium'`);
  } catch {}
  await connection.query(
    `UPDATE restaurants
